...
bool listener::init()
{
    // Xlib has to know that it is going to be used from more than one thread
    // before any other Xlib call. This display is polled by the listener thread
    // while the application creates and destroys windows from its own thread on
    // the same connection; without this the shared Xlib/XCB state is not
    // protected and the process dies inside xcb_send_request (seen when opening
    // a second window while the first one is being polled).
    XInitThreads();
